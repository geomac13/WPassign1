...
function get_Facebook_data() {
		$.getJSON("https://graph.facebook.com/thinkuhi" , function(fb_data) {
			console.log(fb_data);
			console.log(fb_data['data'][0]['from']['category_list']['name']);
			console.log(fb_data['data'][0]['message'];
...
function get_Google_data() {
	$.ajax({
		type:"GET",
		url:'https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&q=http://uhi.ac.uk/en/media',
		dataType:'jsonp',
		async:false,
...
function get_feed_data() {
		$.getJSON("data.php" , function(feed_data) {
			...
			console.log(feed_data['feed_name'][0]);
			$('#feed-front').append('<h2>' + feed_data['feed_name'][0] + ' writes' + '</h2><p>' + feed_data['feed_review'][0] + '</p>');
		});	
	}